...
class Agent:
	...
	def perturbate_actor(self, state, batch=True):
		...
		if torch.cuda.is_available():
			state = torch.tensor(state, dtype=torch.double).cuda()
		...
		for episode in iterator:
			...
			for t in range(self.env_params['max_timesteps']): 
				...
				for id, (a,r,d,i) in enumerate(zip(action,reward,done,info)):
					experience = ({
                        'achieved_goal':state['achieved_goal'][id],
                        'desired_goal': state['desired_goal'][id],
                        'observation': state['observation'][id] },
                        a,
                        r,
                        d,
                        {
                        'achieved_goal':nextstate['achieved_goal'][id],
                        'desired_goal': nextstate['desired_goal'][id],
                        'observation': nextstate['observation'][id] },
                        i,
                        id )
...
if __name__ == '__main__':
	...
	env_make = tuple(lambda: gym.make('HandManipulateBlock-v0') for _ in range(args.n_processes))
	envs = SubprocVecEnv(env_make)
	env_param = get_params(env)
	agent = Agent(env, envs,env_param,n_episodes=args.n_episodes, n_threads=args.n_processes, save_path=None, \
	batch_size=args.batch_size, tensorboard=args.tb ,her=args.her, per=args.per ,screen=args.render)
	agent.Explore(args.n_batch)
