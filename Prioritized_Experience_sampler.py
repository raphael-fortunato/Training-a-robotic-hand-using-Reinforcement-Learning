...
class prioritized_sampler:
	...
	def getImpact(self, item):
		thing = self.Impact(item[0]["observation"],item[4]["observation"])
		return thing
	...
	def create_sample(self, memory, sample_size, percentage):
		...
		top_sample = list(random.sample(topHalf, int(round(sample_size*percentage, 0))))
		low_sample = list(random.sample(lowestHalf, sample_size-int(round(sample_size*percentage, 0)))) 

		# Concatenate into 1 list and return. 
		return np.concatenate([top_sample, low_sample], axis = 0)